...
export function renderQuestionMarkdown(content) {
  return renderMarkdown(content, ".question-content pre code");
}
...
export function renderAnswerMarkdown(content) {
  return renderMarkdown(content, ".answer-markdown pre code, .analysis-markdown pre code");
}
 ...
 * @returns {string} 渲染后的HTML
 */
export function renderMarkdownWithContainer(content, containerClass) {
  return renderMarkdown(content, `.${containerClass} pre code`);
}
 ...
 * @param {string} selector - CSS选择器
 */
export function clearHighlight(selector = ".markdown-content pre code") {
  document.querySelectorAll(selector).forEach((el) => {
